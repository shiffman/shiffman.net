...
<p>Match URL&#8217;s:</p>
<pre lang="java">%  egrep -i 'http://[^ ]*' a2z.txt</pre>
...
<pre lang="java">%  egrep -i '\< (w+) +\1\>' doubletext.txt</pre>
...
<pre lang="java">import java.util.regex.*</pre>
...
<pre lang="java">Pattern p = Pattern.compile(regex, Pattern.CASE_INSENSITIVE);</pre>
<p>Two flags can be added using the bitwise OR, i.e. | </p>
<pre lang="java">Pattern p = Pattern.compile(regex, Pattern.CASE_INSENSITIVE | Pattern.COMMENTS);</pre>
...
System.out.println("Removed " + count + " other tags."); </pre>
